...
export function ApplicationCreateForm() {
  ...
  if (!open) {
    return <button className="primary-button" onClick={() => setOpen(true)}>+ Add application</button>;
  }
    ...
    <div className="modal-backdrop" role="presentation">
      <section aria-labelledby="new-application-title" aria-modal="true" className="modal" role="dialog">
        <div className="modal-heading">
          <div><p className="eyebrow">Manual tracker</p><h2 id="new-application-title">Add application</h2></div>
        ...
        <form action={action} className="stack-form application-form">
          <div className="form-grid">
            ...
            <label>Status<select defaultValue="APPLIED" name="status">{applicationStatuses.map((status) => <option key={status} value={status}>{titleCase(status)}</option>)}</select></label>
